...
var IIP = new Class({
    ...
    */
    ...
    for( var m=0; m<ntiles; m++ ){
	  ...
	  'events': {
	    ...
	    var src = this.server + "getRegion?format=image/jpeg&level="
	    	    + this.res + "&rotate=0&region="
  ...
  setOpenURL : function() {
    ...
    h = this.hei;
    this.openUrl = this.server + "getRegion?format=image/jpeg&level=" + this.res
        + "&rotate=0&region=" + top_left_y + ","
        + top_left_x + "," + h + "," + w;
   },
  ...
  createWindows: function(){
    ...
    new Element( 'div', {id:'logo'} ).injectInside(this.source);
    new Element('img', {src: 'images/djatoka.32x32.png', id: 'info', styles: { opacity: 0.8 } } ).setProperty('alt','djatoka IIPViewer Info').injectInside('logo');
  ...
  createNavigationWindow: function() {
    ...
    var navimage = new Element('img', {
        id :'navigation',
        src :this.server + "getRegion?format=image/jpeg&scale=" + this.min_x + "," + this.min_y,
    // end djatoka mods
    ...
    navimage.setProperty('alt','Scaled image');
   ...
   var loadBarContainer = new Element('div', {
	...
	id: 'navbuttons',
	html: '<img id="shiftLeft" src="images/left.png" alt="Shift image view left" /><img id="shiftUp" src="images/up.png" alt="Shift image view up" /><img id="shiftRight" src="images/right.png" alt="Shift image view right" /><br/><img id="shiftDown" src="images/down.png" alt="Shift image view down" /><br/><img id="zoomIn" src="images/zoomIn.png" alt="Zoom in" /><img id="zoomOut" src="images/zoomOut.png" alt="Zoom out" /><img id="reset" src="images/reset.png" alt="Reset image" />&nbsp;&nbsp;<img id="snapshot" alt="" width="24" height="24" src="images/djatoka.32x32.png"/>'
    });
  ...
  load : function() {
    new Request( {
        method :'get',
        url :"proxy?url=" + this.server + "getMetadata",
        onComplete : function(transport) {
            var response = transport || alert("No response from server " + this.server);
            var p = eval("(" + response + ")");
            var tmp = p.levels;
